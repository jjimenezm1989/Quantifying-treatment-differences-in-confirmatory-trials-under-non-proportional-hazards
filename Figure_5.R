...
result <- data.table(delay = delay,
                     ...
                     hr_wlr_01 = rep(0, length(delay)),
                     hr_wlr_10 = rep(0, length(delay)),
                     hr_wlr_11 = rep(0, length(delay)),
                     hr_lr = rep(0, length(delay)),
                     rmst_diff = rep(0, length(delay)),
                     rmst_ratio = rep(0, length(delay)),
                     rmst_c = rep(0, length(delay)),
                     rmst_e = rep(0, length(delay)))
...
save.image("~/Dropbox/Research/2019/RMST as an alternative to HR in clinical trials with delayed effects/Journal of Applied Statistics - 1st Review/Figure_4.RData")

load("~/Dropbox/Research/2019/RMST as an alternative to HR in clinical trials with delayed effects/Journal of Applied Statistics - 1st Review/Figure_5.RData")
...
tre = data.frame(delay = rep(result$delay,5),
...
p2 <- ggplot(aes(x = delay, y = treatment_effect, color = test), data = tre) +
  geom_line(size = 1.5) +
  ylim(0.65,0.9) +
  scale_x_continuous("Threshold (months)") +
  labs(y = "Treatment Effect", color = "",title = "B)") +
  scale_color_manual("p'",values=c("#F8766D", "#B79F00", "#00BA38", "#00BFC4", "#619CFF", "#F564E3")) +
  theme_light() +
  theme(text = element_text(size = 16),
...
ggsave("/Users/jose/Dropbox/Research/2019/RMST as an alternative to HR in clinical trials with delayed effects/Journal of Applied Statistics - 1st Review/Figure_5.pdf",scenarios,
