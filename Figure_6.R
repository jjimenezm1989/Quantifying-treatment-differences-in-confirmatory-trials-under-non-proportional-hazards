...
for (i in 1:length(threshold)) {
  ...
  # simulate data
  sim_data_DE <- replicate(M, crossing_hazards_sim(model = model_DE), simplify = FALSE) 
...
load("~/Dropbox/Research/2019/RMST as an alternative to HR in clinical trials with delayed effects/Journal of Applied Statistics - 1st Review/Figure_6.RData")
...
p1 <- ggplot(aes(x = threshold, y = power, color = test), data = df) +
  geom_line(size = 1.5) +
  scale_x_continuous("Threshold (months)", breaks = c(0,3,6,9,12)) +
...
tre$test = factor(tre$test,levels = c("G(0,1)","G(1,1)","LR","RMST","G(1,0)"), labels = c("G(0,1)","G(1,1)","LR","RMST","G(1,0)"),ordered = TRUE)
...
ggsave("/Users/jose/Dropbox/Research/2019/RMST as an alternative to HR in clinical trials with delayed effects/Journal of Applied Statistics - 1st Review/Figure_6.pdf",scenarios,
